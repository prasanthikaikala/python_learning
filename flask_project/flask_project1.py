...
class supplier(db.Model):
    ...
    def __init__(self, name, company_name, phone_no, city, add_line1, add_line2):
        self.name = name
...
def add_supplier():
    if request.method == 'POST':
        if not request.args.get('name') \
                or not request.form['company_name'] \
                or not request.form['phone_no'] \
                or not request.form['city'] \
                or not request.form['add_line1'] \
                or not request.form['add_line2']:
            return jsonify({-1: "Needed form ids in form doesn't exist"})
        else:
            try:
                ...
                new_supplier = supplier(name, company_name, phone_no, city, add_line1, add_line2)
                db.session.add(new_supplier)
                db.session.commit()
                return jsonify({200: "New Supplier successfully added - {supplier}".format(supplier=new_supplier)})
            except Exception as Error:
                print("exception occurred " + str(Error))
                return jsonify({-1: "Failure in Supplier addition " + str(Error)})
    else:
...
class consumer(db.Model):
    ...
    def __init__(self, name, phone_no, city, add_line1, add_line2):
        self.name = name
        self.phone_no = phone_no
        self.city = city
        self.add_line1 = add_line1
...
if __name__ == "__main__":
    with app.app_context():
        db.create_all()
    app.debug = True
    app.run(host="192.168.1.86", port=5080)
